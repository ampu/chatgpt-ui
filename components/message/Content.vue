<template>
  ...
  >
    ...
    />
    <VDivider :color='message.is_bot ? "rgb(var(--v-theme-on-background))" : "rgb(var(--v-theme-on-primary))"'/>
  </VCard>
</template>
...
const md = new MarkdownIt({
  ...
  highlight(code, lang) {
    ...
    const codeText = hljs.highlight(code, {
      language: language,
      ignoreIllegals: true
    })
    return (`
<pre class="hljs-code-container my-3">
    <div class="hljs-code-header d-flex align-center justify-space-between bg-grey-darken-3 pa-1">
        <span class="pl-2 text-caption">${language}</span>
        <button class="hljs-copy-button" data-copied="false">Copy</button>
    </div>
    <code class="hljs language-${language}">${codeText.value}</code>
</pre>
`)
  },
}).use(mathjax3)
...
const bindCopyCodeToButtons = () => {
  ...
  contentElement.value
    ...
    .forEach((container: HTMLElement) => {
      const button = container.querySelector('.hljs-copy-button') as HTMLButtonElement
